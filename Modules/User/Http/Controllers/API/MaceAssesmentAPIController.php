...
class MaceAssesmentAPIController extends AppBaseController
{
    ...
    public function show($id,Request $request)
    {
        ...
        ['HbA1C'=>$HbA1C,'symptom'=>$symptom,'LDL_cholesterol'=>$LDL_cholesterol,'HDL_cholesterol'=>$HDL_cholesterol,'Age'=>$Age,'SBP'=>$SBP,'Triglycerides'=>$Triglycerides,'DBP'=>$DBP,'Heigth'=>$Heigth,'Weigth'=>$Weigth,'Sex'=>$Sex,'Smoker'=>$Smoker,'TBP'=>$TBP,'MI'=>$MI,'Diabetes'=>$Diabetes,'FH'=>$FH,'THL'=>$THL]=$maceAssesment->toArray();
        if((auth()->check() && auth()->user()->panel_type == 6) || ($request->bearerToken() && auth("sanctum")->user()->panel_type == 6)){
            ['LeftAnklePressure'=>$LeftAnklePressure,'RightAnklePressure'=>$RightAnklePressure]=$data;
        }

        if($request->bearerToken()){
            $dataToSave['user_id']=auth("sanctum")->user()->id;
...
$Cond_age="Younger than";
                ...
                if((auth()->check() && auth()->user()->panel_type == 6)  || $request->has("get_new_words")){
                    ...
                    if((auth()->check() && auth()->user()->email == "najafi@vcathlab.com") || (auth()->check() && auth()->user()->email == "mghasemi@vcathlab.com")){
                        ...
                        $patient=Patient::find($mace->patient_id);
                        $template2->setValue("sex",($Sex == 1 ? "Male":"Female"));
                        $template2->setValue("name",patient ? $patient->name:"");
                        $template2->setValue("age",$Age);
                        foreach($template3->getVariables() as $item){
                            if(isset($$item)){
                                $template3->setValue($item,(is_numeric($$item) && (!in_array($item,["Age","DBP","SBP","Vascular_age","Ideal_weight","Height","Weigth"])) ? number_format($$item,2):$$item));
                        ...
                        $template3->setValue("sex",($Sex == 1 ? "Male":"Female"));
                        $template3->setValue("name",$patient ? $patient->name:"");
                        $template3->setValue("age",$Age);
                    }else{
                        ...
                        $template4->setValue("name",$patient ? $patient->name:"");
            ...
            }else if(auth()->user()->email == "kojuri@vcathlab.com"){
                $template=new \PhpOffice\PhpWord\TemplateProcessor(storage_path("app/mace-template4.docx"));
            }
            else{
            ...
            $template->setValue("name",$patient ? $patient->name:"");
    ...
    public function quickPrint(Request $request,$id,$type)
    {
        try{
            $response=$this->show($id,$request);
            $response=$response->original['data'];
            if($type == "abi"){
                $path=$response['result']['links']['bp_triage']['word_link'];
                $path=str_replace(url("/storage"),"",$path);
                $path=storage_path("app/public".$path);
                return response()->download($path);
            }else if($type == "r"){
                $path=$response['result']['links']['ptp_cad']['word_link'];
                $path=str_replace(url("/storage"),"",$path);
                $path=storage_path("app/public".$path);
                return response()->download($path);
            }
