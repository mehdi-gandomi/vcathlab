...
class AngiographyAPIController extends AppBaseController
{
    ...
    public function store(CreateAngiographyAPIRequest $request)
    {
        ...
        ["Cr" => $Cr, "Ht" => $Ht, "LVEF" => $LVEF, "HR" => $HR, "Contrast" => $Contrast, "Hb" => $Hb, "PTP" => $PTP, "CAVI" => $CAVI, "WBC" => $WBC, "HbA1C" => $HbA1C, "patient_id" => $patient_id, "Age" => $Age, "SBP" => $SBP, "DBP" => $DBP, "Height" => $Height, "Weight" => $Weight, "Sex" => $Sex, "pribleed" => $pribleed, "Hypotension" => $Hypotension, "heart_failure" => $heart_failure, "Diabet" => $Diabet, "Acute_MI" => $Acute_MI, "IABP" => $IABP, "Smoker" => $Smoker, "Weight" => $Weight, "Height" => $Height, "PriorCABG" => $PriorCABG, "PriorPCI" => $PriorPCI] = $data;
        ...
        if ($LVEF >= 40 && $LVEF < 55) {
            $Grad_EF = "mildly reduced";
        }

        if ($LVEF >= 30 && $LVEF < 40) {
            $Grad_EF = "moderately reduced";
        }

        if ($LVEF < 30) {
            $Grad_EF = "severely reduced";
        }
