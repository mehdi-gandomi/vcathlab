...
export default {
            ...
            inputs:{
                @foreach ($columns as $column)
                    @if (fieldExistsInForm($model, $column['name']))
                        ...
                        field_type:"{{ $model::$fields[$column['name']]['htmlType']}}",
    ...
    methods: {
        async onSubmit(action){
            const data =await this.form.post("{{ $baseApiPath }}/{{ $modelPlural }}");
            if(data.success){
            Iracode.success(this.__("$TABLE_NAME_TITLE$ Created Successfully"))
