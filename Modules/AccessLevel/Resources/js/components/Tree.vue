...
import Vuex, { mapState, mapMutations } from '@node_modules/vuex';
import * as VueDeepSet from '@node_modules/vue-deepset';
import trans from '@/mixins/trans';
import VueTree from '@node_modules/@riophae/vue-treeselect';
...
export default {
  ...
  created() {
	...
	// console.log('#VueDeepSet', VueDeepSet);
  ...
  async mounted() {
    ...
    const { data } = await this.$http.get(`${window.config.permissions_base_url}/getTree`);
    this.tree = data;
