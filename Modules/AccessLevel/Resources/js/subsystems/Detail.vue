    ...
    <div class="vx-col w-full">
      <vx-card class="mb-base">
        <template v-slot:actions>
          <vs-button color="primary" to="/accesslevel/subsystems" type="filled">Back</vs-button>
        </template>
        <table>
          <tr>
          ...
          <tr>
            <td class="font-semibold">{{ __('State') }}</td>
            <td>{{ details.state }}</td>
          </tr>
        </table>
      </vx-card>
...
export default {
  ...
  async created() {
    const { data: response } = await this.$http.get(
      `/accesslevel/api/subsystems/${this.$route.params.id}`
    );
    if (response.success) {
      const { data } = response;
