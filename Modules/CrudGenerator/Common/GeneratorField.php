...
class GeneratorField
{
    ...
    private function prepareMigrationText()
    {
        ...
        if ($this->fieldType == 'enum') {
            ...
            if(count($fieldTypeParams)) $this->migrationText = substr($this->migrationText, 0, strlen($this->migrationText) - 1);
            $this->migrationText .= ']';
        } else {
            foreach ($fieldTypeParams as $param) {
                $this->migrationText .= ', '.$param;
        ...
        if($this->comment){
            ...
            $this->migrationText.="->comment('".$this->comment."')";
        }
        $this->migrationText .= ';';
    }
    ...
    public static function parseFieldFromFile($fieldInput)
    {
        ...
        $field->htmlValues = isset($fieldInput['htmlValues']) ? $fieldInput['htmlValues'] : '';
        $field->filterOptions = isset($fieldInput['filterOptions']) ? $fieldInput['filterOptions'] : '';
        $field->filepondOptions = isset($fieldInput['filepondOptions']) ? $fieldInput['filepondOptions'] : '';
        ...
        // $field->comment = isset($fieldInput['comment']) ? $fieldInput['comment'] : '';
        $field->label = isset($fieldInput['label']) ? $fieldInput['label'] : '';
        $field->filter = isset($fieldInput['filter']) ? $fieldInput['filter'] : 'text';
        return $field;
    }
