...
use Modules\CrudGenerator\Utils\GeneratorFieldsInputUtil;
            ...
            if ($this->getOption('fieldsFile')) {
                ...
                if (file_exists($fieldsFileValue)) {
                    ...
                    $schemaFileDirector = config(
                        'crudgenerator.path.schema_files',
                ...
                foreach ($jsonData as $field) {
                    if (isset($field['type']) && $field['relation']) {
                        ...
                        $field=GeneratorField::parseFieldFromFile($field);
                        $this->fields[] =$field;
                ...
                foreach ($jsonData['fields'] as $field) {
                    if (isset($field['type']) && $field['relation']) {
                        ...
                        $generatorField=GeneratorField::parseFieldFromFile($field);
                        $generatorField->parseDBType($field['dbType']);
                        $this->fields[] =$generatorField;
                        if (isset($field['relation'])) {
                            $this->relations[] = GeneratorFieldRelation::parseRelation($field['relation']);
                        }
                    }
