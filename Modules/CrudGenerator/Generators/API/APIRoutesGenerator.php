...
class APIRoutesGenerator extends BaseGenerator
{
    ...
    public function __construct(CommandData $commandData)
    {
        ...
        if (!empty($this->commandData->config->prefixes['route'])) {
            $routesTemplate = get_template('api.routes.prefix_routes', 'CrudGenerator');
        } else {
            $routesTemplate = get_template('api.routes.routes', 'CrudGenerator');
        }
    ...
    public function generate()
    {
        ...
        if (Str::contains($existingRouteContents, "Route::crud('".$this->commandData->config->mSnakePlural."',")) {
