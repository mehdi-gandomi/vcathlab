...
namespace Modules\CrudGenerator\Utils;
...
class TableFieldsGenerator
{
    ...
    public function __construct($tableName, $ignoredFields, $connection = '')
    {
        ...
        $mappings = config('crudgenerator.from_table.doctrine_mappings', []);
        ...
        $this->defaultSearchable = config('crudgenerator.options.tables_searchable_default', false);
    }
        ...
        foreach ($this->columns as $column) {
            ...
            switch ($type) {
                ...
                case 'smallint':
                    ...
                    break;
                case 'tinyint':
                    $field = $this->generateIntFieldInput($column, 'tinyInteger');
                    break;
                case 'bigint':
    ...
    {
        if (!config('crudgenerator.timestamps.enabled', true)) {
            return [];
        }

        $createdAtName = config('crudgenerator.timestamps.created_at', 'created_at');
        $updatedAtName = config('crudgenerator.timestamps.updated_at', 'updated_at');
        $deletedAtName = config('crudgenerator.timestamps.deleted_at', 'deleted_at');
