<template>
    <div>
        <div class="flex justify-center my-2">
            <vs-button color="success" @click="print" type="filled"
                >Print</vs-button
            >
        </div>
        <div id="crudPrint">
            <div class="flex justify-between">
                <div class="logo">
                    <img :src="logoUrl" alt="" />
                </div>
                <div class="exportTitle">
                    <h4>{{ title }}</h4>
                </div>
                <div class="date">
                    <strong>{{ __("Date") }}:</strong>
                    <p>{{ currentDate }}</p>
                </div>
            </div>
            <div class="flex justify-between my-5">
                <div class="siteName">
                    <h4>{{ site_name }}</h4>
                </div>
                <div class="exportTitle">
                    <h4>{{ title }}</h4>
                </div>
                <div class="date">
                    <strong>{{ __("Time") }}:</strong>
                    <p>{{ currentTime }}</p>
                </div>
            </div>
            <div class="table-wrap">
                <vs-table stripe :data="items">
                    <template slot="thead">
                        <vs-th>
                            {{ __("Username") }}
                        </vs-th>
                        <vs-th>
                            {{ __("First Name") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Last Name") }}
                        </vs-th>
                        <vs-th>
                            {{ __("State") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Country") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Profession") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Specialty") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Mobile") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Email") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Email Verified At") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Password") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Two Factor Secret") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Two Factor Recovery Codes") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Remember Token") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Created At") }}
                        </vs-th>
                        <vs-th>
                            {{ __("Updated At") }}
                        </vs-th>
                    </template>

                    <template slot-scope="{ data }">
                        <vs-tr :key="indextr" v-for="(tr, indextr) in data">
                            <vs-td :data="data[indextr].username">
                                {{ data[indextr].username }}
                            </vs-td>
                            <vs-td :data="data[indextr].first_name">
                                {{ data[indextr].first_name }}
                            </vs-td>
                            <vs-td :data="data[indextr].last_name">
                                {{ data[indextr].last_name }}
                            </vs-td>
                            <vs-td :data="data[indextr].state">
                                {{ data[indextr].state }}
                            </vs-td>
                            <vs-td :data="data[indextr].country">
                                {{ data[indextr].country }}
                            </vs-td>
                            <vs-td :data="data[indextr].profession">
                                {{
                                    data[indextr].profession
                                        | selectFormatter(
                                            data[indextr],
                                            "profession"
                                        )
                                }}
                            </vs-td>
                            <vs-td :data="data[indextr].specialty">
                                {{
                                    data[indextr].specialty
                                        | selectFormatter(
                                            data[indextr],
                                            "specialty"
                                        )
                                }}
                            </vs-td>
                            <vs-td :data="data[indextr].mobile">
                                {{ data[indextr].mobile }}
                            </vs-td>
                            <vs-td :data="data[indextr].email">
                                {{ data[indextr].email }}
                            </vs-td>
                            <vs-td :data="data[indextr].email_verified_at">
                                {{
                                    data[indextr].email_verified_at
                                        | dateFormatter(
                                            data[indextr],
                                            "email_verified_at"
                                        )
                                }}
                            </vs-td>
                            <vs-td :data="data[indextr].password">
                                {{ data[indextr].password }}
                            </vs-td>
                            <vs-td :data="data[indextr].two_factor_secret">
                                {{ data[indextr].two_factor_secret }}
                            </vs-td>
                            <vs-td
                                :data="data[indextr].two_factor_recovery_codes"
                            >
                                {{ data[indextr].two_factor_recovery_codes }}
                            </vs-td>
                            <vs-td :data="data[indextr].remember_token">
                                {{ data[indextr].remember_token }}
                            </vs-td>
                            <vs-td :data="data[indextr].created_at">
                                {{
                                    data[indextr].created_at
                                        | dateFormatter(
                                            data[indextr],
                                            "created_at"
                                        )
                                }}
                            </vs-td>
                            <vs-td :data="data[indextr].updated_at">
                                {{
                                    data[indextr].updated_at
                                        | dateFormatter(
                                            data[indextr],
                                            "updated_at"
                                        )
                                }}
                            </vs-td>
                        </vs-tr>
                    </template>
                </vs-table>
            </div>
        </div>
    </div>
</template>

<script>
import Printable from "@/mixins/Printable";
export default {
    mixins: [Printable],
    data() {
        return {
            baseUrl: "/admin/api/users",
            title: this.__("Export Users"),
            items: [],
            model: "User",
            module: "Admin",
        };
    },
};
</script>
<style lang="scss">
#crudPrint {
    padding: 5rem;
    max-width: 90%;
    margin: auto;

    .logo {
        width: 100px;
        img {
            width: 100%;
        }
    }
}
</style>
