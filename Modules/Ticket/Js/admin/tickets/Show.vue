<template>
  <loading-card v-if="Object.keys(data).length > 0">
    <vx-card :title="data.ticket.subject" >
        <template slot="actions">
            <div>
                ...
                <vs-button @click="statusPopupActive=true" color="primary" type="border">Change status</vs-button>
            ...
            <vs-row vs-type="flex" vs-w="12">
                ...
                >
                    ...
                    <p>
                        ...
                        <span :style="{color: data.ticket.status_id.color}">{{data.ticket.status_id.name}}</span>
                    </p>
                    <p>
      ...
      <div class="ticket-comments mt-8">
            <h4>{{__("Comments")}}</h4>
            <div>
                <vx-card :class="{'primary-card':(comment.user_id != data.ticket.user_id)}" v-for="(comment,key) in data.comments.data" :key="key" :title="comment.user.name">
                    <template slot="actions">
    ...
    <vs-popup class="holamundo"  :title="__('Change status')" :active.sync="statusPopupActive">
        <vue-select :options="statuses" v-model="updateForm.status_id" :reduce="op => op.id" label="name">

        </vue-select>
        <vs-button @click="changeStatus" class="mt-4" color="primary" type="relief">Change</vs-button>
    </vs-popup>
  </loading-card>
</template>
<script>
export default {
    ...
    async created() {
        ...
        this.updateForm=this.data.ticket;
        const {data:stats}=await this.$http.get("/tickets-admin/status");
        this.statuses=stats.data;
    },
    methods: {
        ...
        async changeStatus(){
            this.$vs.loading();
            const {data:result}=await this.$http.patch(`/tickets/${this.$route.params.id}`,this.updateForm);
            if(result.ok){
                Iracode.success(result.message)
            }
            const {data}=await this.$http.get(`/tickets/${this.$route.params.id}`);
            this.data=data.data;
            this.form.ticket_id=this.data.ticket.id;
