...
namespace Modules\Ticket\Seeds;
...
class TicketitTableSeeder extends Seeder
{
    ...
    public function run()
    {
        ...
        foreach ($this->statuses as $name => $color) {
            $status = \Modules\Ticket\Models\Status::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        ...
        foreach ($this->categories as $name => $color) {
            $category = \Modules\Ticket\Models\Category::create([
                'name'  => $name,
                'color' => $color,
            ]);
        ...
        foreach ($this->priorities as $name => $color) {
            $priority = \Modules\Ticket\Models\Priority::create([
                'name'  => $name,
                'color' => $color,
            ]);
        }
        $categories_qty = \Modules\Ticket\Models\Category::count();
        $priorities_qty = \Modules\Ticket\Models\Priority::count();
        $statuses_qty = \Modules\Ticket\Models\Status::count();
        ...
        for ($u = 1; $u <= $this->users_qty; $u++) {
            ...
            for ($t = 1; $t <= $tickets_qty; $t++) {
                ...
                $category = \Modules\Ticket\Models\Category::find($rand_category);
                ...
                $ticket = new \Modules\Ticket\Models\Ticket();
                ...
                for ($c = 1; $c <= $comments_qty; $c++) {
                    ...
                    $comment = new \Modules\Ticket\Models\Comment();
