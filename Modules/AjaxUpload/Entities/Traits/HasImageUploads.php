...
namespace Modules\AjaxUpload\Entities\Traits;
...
use Modules\AjaxUpload\Exceptions\InvalidUploadFieldException;
...
trait HasImageUploads
{
     ...
     */
    public function uploadImage($imageFile, $field = null,$updateModel=true)
    {
        $this->uploadFieldName = $this->getUploadFieldName($field);
        $this->uploadFieldOptions = $this->getUploadFieldOptions($this->uploadFieldName);
     ...
     * @throws InvalidUploadFieldException|\Exception
     */
    public function uploadImageFromTempPath($imageFile, $field = null)
    {
        $this->uploadFieldName = $this->getUploadFieldName($field);
        $this->uploadFieldOptions = $this->getUploadFieldOptions($this->uploadFieldName);
    ...
    public function getImageUploadPath()
    {
        ...
        return property_exists($this, 'imagesUploadPath')
            ...
            : trim(config('ajaxupload.upload_path', 'uploads'), '/');
    }
     ...
     * @return string
     */
    public function getFileUploadPath($file)
    {
     ...
     * @return string
     */
    public function getImageUploadDisk()
    {
        // check for disk option
     ...
     * @return boolean
     */
    public function canAutoUploadImages()
    {
     ...
     * @param $imageOptions
     */
    public function validateImage($file, $fieldName, $imageOptions)
    {
     ...
     * @throws \Exception
     */
    public function saveImage($imageFile, $image)
    {
        ...
        // clean up
        $image->destroy();
        if(file_exists($imageFile->getPathName())) unlink($imageFile->getPathName());
        return $imagePath;
    }
     ...
     */
    public function getCropOption($imageFieldOptions)
    {
     ...
     * @return bool
     */
    public function needResizing($imageFieldOptions)
    {
     ...
     * @throws \Exception
     */
    public function triggerHook($hook, $image)
    {
        if (is_callable($hook)) {
            $hook($image);
        }
     ...
     * @throws \Exception
     */
    public function triggerBeforeSaveHook($image)
    {
     ...
     * @throws \Exception
     */
    public function triggerAfterSaveHook($image)
    {
