    ...
    @mouseout  = "mouseout">
      ...
      <div @click="clickGroup" class="group-header w-full">
        <span class="flex items-center w-full">
          ...
          <img
            v-if        = "group.icon_url  || (this.groupIndex > Math.floor(this.groupIndex))"
            style="    margin-right: 14px;    margin-left: -5px;"
            :style="    {'width': group.width || '25px'}"
            :src="group.icon_url"
             />

          <!-- Group Name -->
          <span v-show="!verticalNavMenuItemsMin" class="truncate mr-3 select-none">{{ __(group.name) }}</span>
        ...
        <!-- Group Tooltip -->
        <span class="vs-sidebar--tooltip">{{ __(group.name) }}</span>
      </div>
      <!-- /Group Label -->
        ...
        <li v-for="(groupItem, index) in group.submenu" :key="index">
          ...
          <v-nav-menu-item
            ...
            :target = "groupItem.target">
              <span class="truncate">{{ __(groupItem.name)  }}</span>
