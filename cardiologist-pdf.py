...
# AI System Prompt for Hematology Analysis
system_prompt = """
...
# Function to analyze extracted CBC report
def analyze_cbc_report_from_text(cbc_text):
    client = openai.OpenAI(api_key=api_key)

    user_input = f"""
    Please analyze the following CBC (Complete Blood Count) report extracted from a PDF:

    {cbc_text}

    Identify key values (WBC, RBC, Hemoglobin, Platelets, MCV, MCH, etc.) and provide an interpretation.
    """
...
if __name__ == "__main__":
    pdf_path = input("Enter the path to the CBC report PDF: ")

    # Step 1: Extract text from PDF
    extracted_text = extract_text_from_pdf(pdf_path)
    print("\n📜 Extracted Text from PDF:\n", extracted_text)
    exit()
    # Step 2: Get AI-generated hematology analysis
    result = analyze_cbc_report_from_text(extracted_text)
